...
@objc open class AKSettings: NSObject {
    ...
    @objc public static var numberOfChannels: UInt32 = 2
    ...
    @objc public static var rampTime: Double = 0.000_2
...
extension AKSettings {
  ...
  /// Shortcut for AVAudioSession.sharedInstance()
    ...
    @objc public static func setSession(category: SessionCategory,
                                with options: AVAudioSession.CategoryOptions = [.mixWithOthers]) throws {

        if ❗️AKSettings.disableAVAudioSessionCategoryManagement {
            do {
                try AKTry {
                    try session.setCategory(AVAudioSession.Category(rawValue: category.description), options: options)
                }
    ...
    @objc static public var headPhonesPlugged: Bool {
        return false
        return session.currentRoute.outputs.contains {
            [convertFromAVAudioSessionPort(AVAudioSession.Port.headphones),
             convertFromAVAudioSessionPort(AVAudioSession.Port.bluetoothHFP),
             convertFromAVAudioSessionPort(AVAudioSession.Port.bluetoothA2DP)].contains(convertFromAVAudioSessionPort($0.portType))
        }
    }
    ...
    @objc public enum SessionCategory: Int, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .multiRoute:
                return convertFromAVAudioSessionCategory(AVAudioSession.Category.multiRoute)
            case .audioProcessing:
                #if !os(tvOS)
                    return convertFromAVAudioSessionCategory(AVAudioSession.Category.audioProcessing)
                #else
